class HomeController < ApplicationController
  ...
  def index
    if current_user
        session[:battle_uids] = current_user.random_match('female')
    end
  ...
  def battle
    ...
    if params[:choice] == "right" then @dscore = User.update_win_loss_by_uid(session[:battle_uids][0]['uid'], session[:battle_uids][1]['uid']) end
